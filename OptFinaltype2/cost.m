...
function J=cost(gains,t,type)
    ...
    for k=2:length(t)
        ...
        if k>3
            u(k)=u(k-1)+(gains(1)+gains(3)*dt+gains(2)/dt)*error(k)+(-2*gains(2)/dt-gains(1))*error(k-1)+(gains(2)/dt)*error(k-2);
        else
            u(k)=u(k-1)+(gains(1)+gains(3)*dt+gains(2)/dt)*error(k)+(-2*gains(2)/dt-gains(1))*error(k-1);
        end
        if u(k)>3.3
            u(k)=3.3;
        end
