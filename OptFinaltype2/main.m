...
for k=2:length(t)
    ...
    if k>3
        u(k)=u(k-1)+(sol(1)+sol(3)*dt+sol(2)/dt)*error(k)+(-2*sol(2)/dt-sol(1))*error(k-1)+(sol(2)/dt)*error(k-2);
    else
        u(k)=u(k-1)+(sol(1)+sol(3)*dt+sol(2)/dt)*error(k)+(-2*sol(2)/dt-sol(1))*error(k-1);
    end
    if u(k)>3.3
        u(k)=3.3;
    end
